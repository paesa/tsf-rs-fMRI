...
tol = 0.0031
folder = "matrices/"
filename = "matrices/32_g7s16.txt"
...
for filename in group:
    ...
    m = go.load_incidence_matrix_tol(folder + filename, tol=tol)
    ending = filename.replace('.txt', '')
    result_folder = "results/" 
    df = go.obtain_topological_properties(m, 
            label="TP_" + ending, folder=result_folder)
